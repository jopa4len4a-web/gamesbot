...
bot.onText(/\/create/, (msg) => {
  ...
  bot.sendMessage(chatId,
    `🏗️ Игра создана!\n\nКатастрофа: *${games[chatId].catastrophe}*\n\nИгроки присоединяются командой /join. Создатель начинает игру командой /startgame.`,
    { parse_mode: 'Markdown' }
  );
});
...
bot.onText(/\/join/, (msg) => {
  ...
  bot.sendMessage(chatId,
    `✅ *${player.username}* присоединился. (${game.players.length} игроков)`,
    { parse_mode: 'Markdown' }
  );
});
...
bot.onText(/\/status/, (msg) => {
  const chatId = msg.chat.id;
  const game = games[chatId];
  if (!game) { bot.sendMessage(chatId, '❌ Нет активной игры.'); return; }
  const list = game.players.map(p => `${p.alive ? '🟢' : '🔴'} ${p.username}`).join('\n');
  bot.sendMessage(chatId,
