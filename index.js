// index.js — Бункер (финальная версия со всеми патчами)
...
async function startBot() {
  try {
    ...
    console.error('❌ Ошибка старта бота:', err.message);
    process.exit(1);
  }
}
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/api/state') {
    ...
    if (!chatId) {
      res.writeHead(400);
      res.end(JSON.stringify({ error: 'chatId не указан' }));
      return;
    }

    if (!userIdRaw) {
      res.writeHead(400);
      res.end(JSON.stringify({ error: 'userId не указан' }));
      return;
    }
    ...
    if (isNaN(userId)) {
      res.writeHead(400);
      res.end(JSON.stringify({ error: 'userId не число' }));
      return;
    }
    ...
    if (!game) {
      console.log(`[API /state] chatId=${chatId} — игра не найдена. Ключи: ${Object.keys(games).join(', ') || 'нет'}`);
      res.writeHead(404);
    ...
    console.log(`[API /state] chatId=${chatId} userId=${userId} найден=${!!player} игроков=${game.players.length}`);
    ...
    res.end(JSON.stringify({
      ...
      debug: {
        ...
        playerFound: !!player,
        playerIds: game.players.map(p => ({ userId: p.userId, username: p.username }))
      },
...
function lsBotUrl(chatId) {
  if (!BOT_USERNAME) {
    console.warn('⚠️ BOT_USERNAME ещё не готов');
    return `https://t.me/?start=join_${chatId}`;
  }
  ...
  if (msg.chat.type === 'private') {
    if (payload.startsWith('join_')) {
      ...
      bot.sendMessage(chatId,
        ...
        `⏳ Ждём старта. Как только хост запустит игру — я пришлю тебе карточку.`,
        { parse_mode: 'Markdown' }
      );
      return;
    }
...
bot.onText(/\/newgame/, async (msg) => {
  ...
  bot.sendMessage(chatId,
    ...
    `👇 Жми *«Вступить»* — бот откроет личку, где ты будешь ждать старта.`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '✋ Вступить в игру', url: lsBotUrl(chatId) }],
...
// ==========================================
// ЗАПУСК ИГРЫ (ПАТЧ 1: защита от двойного запуска)
// ==========================================
async function launchGame(chatId) {
  const game = games[chatId];
  if (!game) return;
  if (game.state !== 'lobby') {
    console.log(`[launchGame] Игра ${chatId} уже в состоянии ${game.state}, пропускаем`);
    return;
  }
  ...
  bot.sendMessage(chatId,
    ...
    `👥 *Игроки:*\n${playerList(game)}\n\n` +
    `📍 *Раунд 1*\n\n` +
    `📱 *Каждый игрок получает карточку в ЛС с ботом.*\n` +
    `👇 Если не получил — нажми кнопку:`,
    {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
          [{ text: '📩 Открыть ЛС с ботом', url: `https://t.me/${BOT_USERNAME}` }]
        ]
      }
    }
  );
  ...
  for (const player of game.players) {
    try {
      ...
      bot.sendMessage(chatId,
        `⚠️ *${escapeMd(player.username)}*, открой ЛС с ботом!`,
        { parse_mode: 'Markdown' }
      ).catch(() => {});
    }
  }
...
// ==========================================
// ГОЛОСОВАНИЕ (ПАТЧ 2: победа от 2 игроков)
// ==========================================
async function resolveVote(chatId) {
  ...
  // ПАТЧ 2: победа только если игроков >= 2
  if (game.players.length >= 2 && aliveCount <= Math.ceil(game.players.length / 2)) {
    game.state = 'finished';
