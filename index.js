// index.js — Бункер (версия с ЛС-кнопкой и отладкой)
...
// Узнаём username бота при старте
bot.getMe().then(me => {
  BOT_USERNAME = me.username;
  console.log(`🤖 Бот: @${BOT_USERNAME}`);
}).catch(err => console.error('Ошибка getMe:', err.message));
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/api/state') {
    ...
    if (!game) {
      res.writeHead(404);
      res.end(JSON.stringify({ error: 'Игра не найдена', chatId, userId }));
      return;
    }
  ...
  // ===== /api/whoami =====
  // Возвращает userId и chatId из initData (для отладки)
  if (pathname === '/api/whoami' && req.method === 'POST') {
    let body = '';
    req.on('data', chunk => body += chunk);
    req.on('end', () => {
      try {
        const { initData, chatId } = JSON.parse(body);
        ...
        res.end(JSON.stringify({
          ...
          chatId,
          game: games[chatId] ? {
            state: games[chatId].state,
            players: games[chatId].players.map(p => ({
              userId: p.userId,
              username: p.username
            }))
          } : null
...
function lsBotUrl(chatId) {
  if (!BOT_USERNAME) return `https://t.me/?start=join_${chatId}`;
...
async function launchGame(chatId) {
  ...
  // Сообщение в группу с кнопкой "Открыть ЛС с ботом"
  bot.sendMessage(chatId,
    `🎬 *ИГРА НАЧАЛАСЬ!*\n\n` +
    ...
    `👇 Если не получил — нажми кнопку:`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          ...
          [{ text: '🗳️ Голосовать', url: webappUrl(chatId) }]
        ]
      }
    }
  );
